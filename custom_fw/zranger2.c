...
void zRanger2Task(void* arg)
{
  ...
  //Simon
  //VL53L1_Error status = VL53L1_ERROR_NONE;
  //VL53L1_DetectionConfig_t detConf; 
  ...
  //SIMON
  /*
  VL53L1_GetThresholdConfig(&dev, &detConf);
  detConf.DetectionMode = 1;
  detConf.Distance.CrossMode = 3;
  detConf.IntrNoTarget = 0;
  detConf.Distance.High = 3000;
  detConf.Distance.Low = 1;
  VL53L1_SetThresholdConfig(&dev, &detConf);
  */
  VL53L1_CalibrationData_t calData;
  VL53L1_GetCalibrationData(&dev, &calData);
  calData.customer.mm_config__inner_offset_mm = 0;
  calData.customer.mm_config__outer_offset_mm = 0;
