...
// SIMON
void pwm3901EnableFrameBuffer(uint32_t csPin)
{
  
  registerWrite(csPin, 0x7F, 0x07);  //Magic frame readout registers
  registerWrite(csPin, 0x41, 0x1D);
  registerWrite(csPin, 0x4C, 0x00);
  registerWrite(csPin, 0x7F, 0x08);
  registerWrite(csPin, 0x6A, 0x38);
  registerWrite(csPin, 0x7F, 0x00);
  registerWrite(csPin, 0x55, 0x04);
  registerWrite(csPin, 0x40, 0x80);
  registerWrite(csPin, 0x4D, 0x11);

  registerWrite(csPin, 0x70, 0x00);   //More magic? 
  registerWrite(csPin, 0x58, 0xFF);

  int temp = registerRead(csPin, 0x58); //Read status register 
  int check = temp>>6; //rightshift 6 bits so only top two stay

  while(check == 0x03){ //while bits aren't set denoting ready state
    temp = registerRead(csPin, 0x58); //keep reading and testing 
    check = temp>>6; //shift again 
  }  
  sleepus(50);
}

void pwm3901ReadFrameBuffer(uint32_t csPin, int *FBuffer)
{
  int count = 0;
