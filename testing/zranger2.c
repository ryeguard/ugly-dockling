 ...
 * Kistoffers kod för samplingsfrekvenstesting
 */
...
static STATS_CNT_RATE_DEFINE(sampleRate, 1000);
...
void zRanger2Task(void* arg)
{
  ...
  while (1) {
    ...
    STATS_CNT_RATE_EVENT(&sampleRate);
...
STATS_CNT_RATE_LOG_ADD(smplRate, &sampleRate)
